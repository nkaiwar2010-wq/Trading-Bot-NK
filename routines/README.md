...
|---|---|---|---|
| mirage-trigger-daemon.md | Claude Code cloud routine | `30 13 * * 1-5` UTC (8:30am Chicago) | The ONLY thing that starts the daemon — pushes a commit touching `.github/mirage-trigger.txt`, which the workflow below watches. Exists because two other paths failed: GitHub Actions' `schedule:` cron was unreliable (fired 5.5hrs late day 1, didn't fire at all day 2 — no SLA, documented GitHub behavior), and calling GitHub's REST API directly (`workflow_dispatch` via curl) was blocked at the Claude Code session/proxy level (needs a GitHub App org connection this project doesn't have). Plain `git push` needs no special scope beyond what every other routine already uses. |
| `.github/workflows/mirage-daemon.yml` → `scripts/mirage_daemon.py` | GitHub Actions (`push` to `.github/mirage-trigger.txt`, plus `workflow_dispatch` for manual runs) | Triggered by the routine above | Manage open positions (VWAP-loss/target exit), screen movers/most-actives, confirm ORB, trade if it clears the checklist. Commits only on an actual entry/exit. Requires repo secrets `MIRAGE_ALPACA_API_KEY` / `MIRAGE_ALPACA_SECRET_KEY`. |
