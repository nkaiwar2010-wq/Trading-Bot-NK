...
|---|---|---|---|
| mirage-trigger-daemon.md | Claude Code cloud routine | `30 13 * * 1-5` UTC (8:30am Chicago) | The ONLY thing that starts the daemon — calls GitHub's API to fire a `workflow_dispatch` event. Exists because GitHub Actions' own `schedule:` cron proved unreliable in practice (fired 5.5hrs late day 1, didn't fire at all day 2 — no SLA on scheduled workflows, documented GitHub behavior). Requires the GITHUB_TOKEN in Mirage's environment to have "Actions: Read and write" permission in addition to Contents. |
| `.github/workflows/mirage-daemon.yml` → `scripts/mirage_daemon.py` | GitHub Actions (`workflow_dispatch` only, no `schedule:`) | Triggered by the routine above | Manage open positions (VWAP-loss/target exit), screen movers/most-actives, confirm ORB, trade if it clears the checklist. Commits only on an actual entry/exit. Requires repo secrets `MIRAGE_ALPACA_API_KEY` / `MIRAGE_ALPACA_SECRET_KEY`. |
